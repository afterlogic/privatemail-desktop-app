...
import Settings from 'src/modules/openpgp/settings'
...
CCrypto.prototype.prepareKey = async function (oFile, sParanoidEncryptedKey = '') {
  ...
  if (sParanoidEncryptedKey) {
    sKey = await OpenPgp.getAesKey(oFile)
    if (!sKey) {
      return false
    }
  }
...
CDownloadFile.prototype.init = async function (oFile, iv, iChunkSize, fProcessBlobErrorCallback, sKey) {
  ...
  this.sDownloadLink = oFile?.Actions?.download?.url
  this.iCurrChunk = 0;
...
{
  console.log(oDecryptedUint8Array, 'oDecryptedUint8Array')
  ...
  oReq.onload = _.bind(function () {
    ...
    if (oReq.status === 200 && oArrayBuffer) {
      ...
      if (this.iCurrChunk !== this.iChunkNumber) {// for all chunk except last - add padding
        ...
        console.log(oArrayBuffer, 'oArrayBuffer')
